...
// Định nghĩa các props mà SelectGroupTwo sẽ nhận vào
interface SelectGroupTwoProps {
  title?: string;
  options: { id: string; name: string }[]; // Giả sử options là một mảng đối tượng với id và name
  key_obj?: string;
  value: string;
  form: any;  // Loại của form (có thể thay bằng kiểu dữ liệu chính xác)
  setForm: (form: any) => void; // Hàm để cập nhật form
  disabled?: boolean; // Tùy chọn để vô hiệu hóa select
}
        ...
        >
          ...
          {props.options.length > 0 &&
            props.options.map((item, key) => (
              <option key={key} value={item.id} className="text-body dark:text-bodydark p-2">
                {item.name}
              </option>
            ))}
        </select>
