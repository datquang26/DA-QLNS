...
import { Button, Checkbox, DatePicker, Form, message, Modal, notification  } from "antd";
...
const ModalCheckIn = () => {
	...
	const handleOk = async () => {
		...
		console.log("respoinse-----1212---------------> ", response)
			
			message.success('Chấm công thanh công!');
			form.resetFields();
			setOpen(false);
			...
			{loading && <Loader className={"bg-opacity-60 bg-white z-50 fixed top-0 left-0 w-full h-full"} />}
				...
				>
					<Form.Item label="Thời gian" name={'check_in'}
						...
						{/* <DatePicker disabledDate={disableDates} format={'DD-MM-YYYY'} placeholder="Chọn thời gian" className="w-full" /> */}
